...
import { searchStandardsFromWeb } from '@/services/standardService';
...
export default function StandardSearch() {
  ...
  const performSearch = async () => {
    ...
    try {
      // 先从本地数据搜索
      const localResults = searchStandards(keyword, filters);
      ...
      if (localResults.length > 0) {
        ...
        // 如果本地没有结果，尝试从网络搜索
        toast.info('正在从网络搜索标准...');
        const webResults = await searchStandardsFromWeb(keyword);
        
        if (webResults.length > 0) {
          setSearchResults(webResults);
          setIsOnlineSearch(true);
          toast.success(`从网络找到 ${webResults.length} 条标准`);
        } else {
          setSearchResults([]);
          toast.info('未找到相关标准，请尝试其他关键词');
        }
      }
    } catch (error) {
